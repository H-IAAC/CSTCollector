...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Check service state
        startBtn.setChecked(ForegroundService.isRunning);

        //IntentService start with 5 random number toasts
        startBtn.setOnClickListener(view -> {

            if(!ForegroundService.isRunning && !startBtn.isChecked()) {
                // When service stops due an error, or clicked in notification
                Toast.makeText(getApplicationContext(), "Service not running!", Toast.LENGTH_SHORT).show();
            } else if(ForegroundService.isRunning) {
                // Send message to stop Handler (stop data collector)
                Intent stopIntent = new Intent();
                stopIntent.putExtra(HANDLER_MESSAGE.COMMAND, HANDLER_ACTION.STOP);
        ...
        statReportBtn.setOnClickListener(view -> {
        ...
        protocolBtn.setOnCheckedChangeListener((buttonView, isChecked) -> preferences.setProtocol(isChecked));
